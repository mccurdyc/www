...
4. Manually Update Fastly Service
5. Manually Update Google Cloud DNS Configuration
...
Run the following to enable GCP DNS and Compute. I was getting 403 errors prior
to enabling these.
...
### Create a "Service" on Fastly
  ...
  1. Create an API token (`Account > Tokens`
  2. Manually Force TLS and HSTS
  3. Manually `HTTPS and networking`

  >>> You can also verify ownership by moving all traffic on this domain to Fastly
  Create a `CNAME` record for `dev.mccurdyc.dev` and point it to `d.sni.global.fastly.net`.
...
_Adding HTTPS to your domain is easy, but is not free. It requires you to attach a credit
...
in Fastly uses `www.mccurdyc.dev`, the TLS configuration in Fastly needs to be `mccurdyc.dev`.

After enabling HTTPS, you will need to add a `CNAME` and `TXT` DNS records in your domain registrar.
